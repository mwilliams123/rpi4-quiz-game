...
def draw_score(screen, rect, player, timer, active):
    ...
    text = Font.number.render('$' + str(player.score), True, Colors.WHITE)
    text_rect = text.get_rect(center=((rect[0] + rect[2])/2, rect[1] + rect[3]*2/3))
    screen.blit(text,text_rect)

    # Draw player identifier
    text = Font.number.render('Player ' + str(player.number + 1), True, Colors.WHITE)
...
def display_score(screen, player_manager):
    ...
    for i, player in enumerate(player_manager.players):
        rect = (0, i*box_height, width, box_height) # box for player's score
